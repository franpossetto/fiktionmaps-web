...
const DeleteFictionModal: React.FC<LogoutModalProps> = ({
  modalOpen,
  setModalOpen,
  fictionToDelete,
  setFictions,
    ...
    const { deleteFiction } = useFictionService();

const _deleteFiction = () => {
    console.log("Delete Fiction");
    deleteFiction(fictionToDelete.id)
    .then(() => {
    console.log('Fiction delete successfully');
    setFictions((prevFictions: any[]) => prevFictions.filter(fiction => fiction.id !== fictionToDelete.id));
})
.catch((error) => {
    console.error('Error deleting fiction');
})
.finally(() => {
    setModalOpen(false)
});
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Are you sure you want to delete the fiction {fictionToDelete?.name} ?
                      </p>
                    </div>
                  </div>
                </div>
...
export default DeleteFictionModal;
